...
export async function GET(
  ...
  // Código ya generado por Supabase
    ...
    const urlVerificacion = `https://ryltraining.cl/verificar?codigo=${encodeURIComponent(
      codigo
    )}`;
  ...
  // Texto de verificación y URL base
  ...
  // QR con la URL de verificación (incluye el código como query param)
  const qrX = 120;
  const qrY = 60;
  const qrPngBytes = await QRCode.toBuffer(urlVerificacion, {
    width: 90,
    margin: 1,
  });
  ...
  const footerText =
    "Documento emitido electrónicamente a través de R&L Training • www.ryltraining.cl";
